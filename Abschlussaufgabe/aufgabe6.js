...
(function (abschluss2) {
    ...
    var clicked = 0;
    var maxMun = 6;
    var municolor = "hsl(" + Math.random() * 360 + ", 100%, 50%)";
    var munizahl = 0;
    var counter = 2000;
    var counterID;
    //var value:boolean[] = [false,false,false,false,false,false];
    var image;
    function shooter() {
        ...
        punkteAnzeige.innerText = punkte.toString();
        //Himmel
        ...
        for (let i = 0; i < 10; i++) {
            let s = new abschluss2.Wolke(0, 0); //Instanz der Klasse wird erstellt
            shapes.push(s); //Wolke wird in das Array geladen
            s.start();
        }
        var xOffset = 950;
        for (let i = 0; i < maxMun; i++) {
            let s = new abschluss2.Munition(xOffset, 0); //Instanz der Klasse wird erstellt
            muni.push(s);
            s.draw();
            xOffset = xOffset + 50;
        }
        addListener();
        startCountdown();
        image = abschluss2.crc2.getImageData(0, 0, 1280, 900); //Bild speichern
        animate();
    }
    function startCountdown() {
        ...
        counterID.style.right = "20px";
        counterID.style.top = "10px";
    ...
    function animate() {
        abschluss2.crc2.clearRect(0, 0, 1280, 900); // hier Hintergrund restaurieren
