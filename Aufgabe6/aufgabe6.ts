namespace aufgabe6 {
    ...
    function ski2(): void {
        ...
        for (let i: number = 0; i < 1; i++) {
            let s: Lift = new Lift(0, 100); //Instanz der Klasse wird erstellt, new übergibt die Werte an den Constructor der Klasse Lift                             
            shapes.push(s);

        }

        for (let i: number = 0; i < 3; i++) {
            let s: Ski = new Ski(0, 230); //Instanz der Klasse wird erstellt, new übergibt die Werte an den Constructor der Klasse Ski
            shapes.push(s);
        }


        for (let i: number = 0; i < 140; i++) {
            let s: Schnee = new Schnee(0, 0); //Instanz der Klasse wird erstellt, new übergibt die Werte an den Constructor der Klasse Schnee
            shapes.push(s);
        }
